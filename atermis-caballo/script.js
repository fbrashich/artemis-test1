 ...
 */
function undoMove() {
    if (isGameOver || history.length <= 1 || undoCount <= 0) return;
...
function checkGameStatus() {
    ...
    // Victoria: visitó todas las casillas (el historial tiene 64 elementos)
    if (history.length === TOTAL_CELLS) {
        isGameOver = true;
...
function updateUI() {
    ...
    btnUndo.disabled = (history.length <= 1 || undoCount <= 0 || isGameOver);
}
